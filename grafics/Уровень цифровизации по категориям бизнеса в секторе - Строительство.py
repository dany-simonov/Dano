...
construction_types = types_df[types_df['type'] == 'Строительство']
merged_df = pd.merge(russia_2019_df, construction_types, on='d1a1x', how='inner')
...
print("Уровень цифровизации по категориям бизнеса:")
for index, row in average_digitalization.iterrows():
    print(f"{row['Категория']}: {row['COVc5']:.2f}%")

print(f"Общий уровень цифровизации: {overall_digitalization:.2f}%")

plt.figure(figsize=(12, 6))
plt.bar(average_digitalization['Категория'], average_digitalization['COVc5'], color='blue')
plt.title('Уровень цифровизации по категориям бизнеса в секторе "Строительство"', fontsize=16)
